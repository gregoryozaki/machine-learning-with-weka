...
## 1. Etapa 1 — Verificação de Integridade Estrutural
...
O Weka exibiu **30 atributos**, **sendo 29 atributos + 1 classe-alvo**. confirmando a estrutura esperada da base.
...
![Evidência 1](https://github.com/user-attachments/assets/31e8fb8a-24d6-4d4f-88a5-f4715bb9f9c8).

#### 5. Tipos dos atributos

Os atributos numéricos foram reconhecidos como **numeric** e os atributos categóricos como **nominal**.

![Evidência 2](https://github.com/user-attachments/assets/2c6a5492-dbc8-46a3-b3d3-69fb9a3d845f).
...
![Evidência 3](https://github.com/user-attachments/assets/dfb3385a-9ab8-40b6-8129-076df3147c81).

- `fan_speed_rpm` → `Missing: 7 (1%)`.

![Evidência 4](https://github.com/user-attachments/assets/8d3217a3-762a-4771-b285-fbd22d666a8c).

- `gpu_temperature_c` → `Missing: 7 (1%)`.

![Evidência 5](https://github.com/user-attachments/assets/7a1562aa-69b0-46c8-a4af-2cbde9323226).
...
![Evidência 6](https://github.com/user-attachments/assets/abeeeb25-07c0-4a86-9830-f13c72be01ef).
...
|---|---|---|---|
| 1 | O dataset abriu corretamente no Weka? | Sim. | O print do Weka mostra a base carregada, com `Instances: 674` e `Attributes: 30`, sem erro de abertura. |
...
Foram identificados valores ausentes em alguns atributos como já foi identificado acima.
...
## 2. Etapa 2 — Análise Estatística Descritiva
...
Os atributos numéricos da base apresentam comportamento compatível com um cenário de monitoramento técnico e operacional de datacenter, com diferentes escalas e amplitudes entre si. No Weka, esses atributos foram reconhecidos como `Numeric` e exibiram estatísticas como mínimo, máximo, média, desvio-padrão, quantidade de valores distintos e valores ausentes, o que permite uma leitura descritiva inicial consistente.

Na base de dados verificada, `water_usage_effectiveness` apresentou mínimo de 0.28, máximo de 4.96, média de 1.246 e desvio-padrão de 0.635. Isso sugere valores concentrados em uma faixa relativamente controlada, com dispersão moderada e sem indícios imediatos de valores extremos incompatíveis com o contexto da variável.
...
![Evidência 7](https://github.com/user-attachments/assets/542fe540-1265-48b5-9e6e-117cc18a1263)

![Evidência 8](https://github.com/user-attachments/assets/73463b1f-ebf0-4bdc-b626-0b942b6ad75d)

![Evidência 9](https://github.com/user-attachments/assets/ffecd974-ca04-4891-8040-db62d5a47ef4)
...
![Evidência 10](https://github.com/user-attachments/assets/db60a650-6c7c-4632-b43b-f1f8e3d5d250)


### Valores faltantes e comportamento suspeito
A base apresenta valores ausentes pontuais em alguns atributos, como `water_usage_effectiveness`, `fan_speed_rpm`, `gpu_temperature_c`, `carbon_intensity_gco2_kwh` e `job_status`. Nos exemplos já verificados, a proporção de ausências foi baixa, em torno de 1% da base, o que não compromete a estrutura geral do dataset, mas exige tratamento adequado no pré-processamento.
...
## 3. Etapa 3 — Análise de Valores Faltantes
...
- Os faltantes ocorrem em atributos importantes para o modelo? **Sim.**
...
## 4. Etapa 4 — Análise de Ruído

### Objetivo

Verificar se o ruído introduzido no dataset é leve, plausível e compatível com o domínio.
...
## 6. Etapa 6 — Análise da Classe-Alvo
...
## 7. Etapa 7 — Análise dos Atributos Irrelevantes
...
## 8. Etapa 8 — Análise de Relações Semânticas

### Objetivo

Verificar se as principais regras semânticas usadas na geração do dataset aparecem de forma consistente nos dados.
...
| Situação do job × duração | Falhas longas podem indicar desperdício.  | Relação interpretável. | Avaliar caso a caso. |
...
![Evidência 11](https://github.com/user-attachments/assets/9e2e2e94-4753-440d-9cc9-d9cdd5606bcd)

![Evidência 12](https://github.com/user-attachments/assets/9d15771f-767f-41fe-a965-15ea9a8aa5b5)