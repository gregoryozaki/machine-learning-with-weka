...
Conduzir um mapeamento sistemático integrado da literatura para fundamentar o desenvolvimento de um dataset sintético voltado à **classificação do nível de desperdício ambiental em racks de datacenter de IA**.
...
|---|---|---|
| **Eixo 1 - Estudo do problema** | Desperdício ambiental em datacenters de IA | Fundamentar o problema, o domínio e a lacuna da pesquisa |
...
| **QP1.4** | Quais técnicas computacionais são aplicadas para analisar, prever, classificar ou detectar ineficiências em datacenters e seus algoritmos de ML para tarefas de classificação? |
...
* ACM Digital Library;
* arXiv;
* Google Scholar como buscador opcional
...
O SciSpace será usado como principal ferramenta para:
...
O SciSpace será o **CS1** deste MSL. Os prompts utilizados na ferramenta serão documentados em [`/prompts/mapeamento_sistematico/SciSpace`](/prompts/mapeamento_sistematico/SciSpace/).

---

### 8.2. Uso do NotebookLM

O NotebookLM será utilizado como principal ferramenta para realização do **CS2**, a extração de dados.
...
As decisões finais de inclusão, exclusão, interpretação e síntese serão realizadas pelos autores. Os prompts utilizados na ferramenta serão documentados em [`/prompts/mapeamento_sistematico/NotebookLM`](/prompts/mapeamento_sistematico/NotebookLM/).
...
* **B) Dados relacionados ao objetivo do eixo**
* **C) Dados para análise crítica**
...
## 10. Resultados e Documentação

Não será elaborada uma documentação oficial específica para os resultados do mapeamento sistemático da literatura, visto que, neste primeiro trabalho, ele assume um caráter mais próximo de uma revisão exploratória, sendo este protocolo usado como guia da pesquisa. Os resultados já serão apresentados nas próprias documentações deste repositório.
...
A integração dos três eixos servirá como base para a etapa seguinte do trabalho: **geração de dataset sintético com LLM** encontrado na pasta [`dataset`](/dataset/README.md) deste repositório.
